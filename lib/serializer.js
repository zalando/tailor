...
module.exports = class CustomSerializer extends Serializer {
    ...
    _serializeNode(currentNode) {
        if (this.addPlaceholders && !this.isPipeInserted && this._isPipeNode(currentNode)) {
        ...
        if (this.addPlaceholders && !this.lastChildInserted && this._isLastChildOfBody(currentNode)) {
            this._serializeRest();
        }
    }
