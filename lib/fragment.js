 ...
 * @returns {object}
 */
const getAttributes = (tag, context, index) => {
...
module.exports = class Fragment extends EventEmitter {
    ...
    insertStart () {
        ...
        if (this.links && this.links['fragment-script']) {
            this.stream.write(`<script data-pipe>${this.pipeInstanceName}.start(${this.index}, "${this.links['fragment-script'].url}", ${JSON.stringify(this.pipeAttributes)})</script>`);
        } else {
    ...
    insertEnd () {
        if (this.links && this.links['fragment-script']) {
            this.stream.write(`<script data-pipe>${this.pipeInstanceName}.end(${this.index}, "${this.links['fragment-script'].url}", ${JSON.stringify(this.pipeAttributes)})</script>`);
        } else {
