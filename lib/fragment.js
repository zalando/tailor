...
module.exports = class Fragment extends EventEmitter {
    ...
    insertStartObserver() {
        // TODO: generate random id properly
        this.lazyFragmentId =
            this.attributes.id ||
            'z' +
                Math.random()
                    ...
                    .substr(2, 9);
        this.stream.write(
            `<div id="${this.lazyFragmentId}" data-js="${this
                .lazyScriptRefs[0]}">`
        );
    }
                    ...
                    const callback = (entries, observer) => {
                        entries.forEach((entry) => {
                            if (entry.intersectionRatio > 0) {
                                const jsAsset = entry.target.getAttribute('data-js');

                                ${this.pipeInstanceName}.loadJS(jsAsset, ${this
                .index});
                    ...
                    const target = doc.querySelector('#${this.lazyFragmentId}');
