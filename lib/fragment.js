...
module.exports = class Fragment extends EventEmitter {
    ...
    fetch(request, isFallback, parentSpan = null) {
        ...
        span.setTag('fragment-url', url);
        span.setTag('fragment-id', this.attributes.id || 'unnamed');
        span.setTag('fallback', isFallback);
        span.setTag('primary', this.attributes.primary || false);
        span.setTag('async', this.attributes.async || false);
        span.setTag('public', this.attributes.public || false);
        ...
        this.requestFragment(url, this.attributes, request, span)
            .then(
                ...
                err => {
                    ...
                    span.setTag(Tags.HTTP_STATUS_CODE, err.statusCode || 0);
                    span.log({
                        message: err.presentable,
                        stack: err.stack
                    });
                    if (!isFallback) {
