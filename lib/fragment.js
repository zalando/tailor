...
module.exports = class Fragment extends EventEmitter {
    ...
    constructor ({
        ...
        this.fragmentID = index / maxAssetLinks;
        this.getPipeAttributes = () => pipeAttributes(Object.assign({}, { id: this.index }, tag.attributes));
    ...
    fetch (request, isFallback) {
        ...
        if (this.attributes.async) {
            return Buffer.from(`<script data-pipe>${this.pipeInstanceName}.placeholder(${this.index})</script>`);
        }
        return this.stream;
    }
    ...
    onResponse (response, isFallback) {
        ...
        const { refs } = LinkHeader.parse([response.headers.link, response.headers['x-amz-meta-link']].join(','));

        this.scriptRefs = refs.filter(ref => ref.rel === 'fragment-script').slice(0, this.maxAssetLinks).map(ref => ref.uri);
        this.styleRefs = refs.filter(ref => ref.rel === 'stylesheet').slice(0, this.maxAssetLinks).map(ref => ref.uri);
