...
const getAttributes = (tag, context) => {
    ...
    const timingGroups = attributes['timing-group'] ? attributes['timing-group'].split(',') : [];
    return {
        url: attributes.src,
        id: fragmentId,
...
module.exports = class Fragment extends EventEmitter {
    ...
    insertStart () {
        ...
        if (this.links && this.links['fragment-script']) {
            this.stream.write(`<script data-pipe>${this.pipeInstanceName}.start(${this.index}, "${this.links['fragment-script'].url}", ${JSON.stringify(this.attributes)})</script>`);
        } else {
    ...
    insertEnd () {
        if (this.links && this.links['fragment-script']) {
            this.stream.write(`<script data-pipe>${this.pipeInstanceName}.end(${this.index}, "${this.links['fragment-script'].url}", ${JSON.stringify(this.attributes)})</script>`);
        } else {
