...
module.exports = class Fragment extends EventEmitter {
    ...
    constructor (tag, context, index, requestFragment) {
        super();
        this.attributes = {
    ...
    fetch (request, isFallback) {
        ...
        if (this.attributes.async) {
            return new Buffer(`<script>p.placeholder(${this.index})</script>`);
        }
        return this.stream;
    }
    ...
    insertStart () {
        if (this.links.stylesheet) {
            if (this.attributes.async) {
                this.stream.write(`<script>p.loadCSS("${this.links.stylesheet.url}")</script>`);
            } else {
                this.stream.write(`<link rel="stylesheet" href="${this.links.stylesheet.url}">`);
            }
        }
        if (this.links && this.links['fragment-script']) {
            this.stream.write(`<script>p.start(${this.index}, "${this.links['fragment-script'].url}")</script>`);
        } else {
            this.stream.write(`<script>p.start(${this.index})</script>`);
        }
    }

    insertEnd () {
        if (this.links && this.links['fragment-script']) {
            this.stream.write(`<script>p.end(${this.index}, "${this.links['fragment-script'].url}")</script>`);
        } else {
            this.stream.write(`<script>p.end(${this.index})</script>`);
        }
    }
