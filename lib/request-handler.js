...
const SWHelper = require('./sw-helper');
const fs = require('fs');
const path = require('path');

module.exports = function processRequest (options, request, response) {
    /* custom service worker file that will be streamed on each request.
    */
    if (request.url === '/sw.js') {
        response.writeHead(200, {
            'Content-Type': 'application/javascript'
        });
        return fs.createReadStream(path.join(__dirname, 'assets.js')).pipe(response);
    }
    // For caching the fragment assets
    const swhelper = new SWHelper();
    this.on('fragment:response', swhelper.onFragmentResponse.bind(swhelper));
    ...
    contextPromise.then((context) => {
        ...
        resultStream.on('alldone', () => {
            swhelper.createSWScript();
        });
