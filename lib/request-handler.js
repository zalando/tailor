...
const PreloadStream = require('./streams/preload-stream');
    ...
    contextPromise.then((context) => {
        ...
        const resultStream = new StringifierStream((tag) => {
            ...
            if (links && links.length) {
                preloads = preloads.concat(links);
                response.setHeader('link', preloads);
            }
        ...
        templatePromise
            .then((template) => {
                ...
                template
                    .pipe(new PreloadStream(options.fragmentTag))
                    .pipe(resultStream);
            })
            .catch((err) => {
                resultStream.emit('error', err);
