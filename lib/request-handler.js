...
module.exports = function processRequest (options, request, response) {
    ...
    const { fetchContext, fetchTemplate, handleTag,
        ...
        dynamicAttribute, fetchDynamicContext,
        pipeAttributes, pipeInstanceName, pipeDefinition,
        maxAssetLinks, filterResponseHeaders } = options;
    ...
    contextPromise.then((context) => {
        ...
        const resultStream = new StringifierStream((tag) => {
            ...
            const isDynamic = dynamicAttribute && tag.attributes && (typeof tag.attributes[dynamicAttribute] !== 'undefined');
            if (name === fragmentTag && isDynamic) {
                ...
                // NOTE: should we expose `tag` to the outside?
                fetchDynamicContext(tag)
                    .then(dynamicContext => {
                        const combinedContext = Object.assign({}, dynamicContext, context);
                        pipeFragmentResult(fetchFragment(combinedContext));
                    })
                    .catch(() => {
                        // NOTE: ideally error handling should
                        // already happen in `fetchDynamicContext`
                        // TODO: emit here same as for global context?
                        pipeFragmentResult(fetchFragment(combinedContext));
                    });
