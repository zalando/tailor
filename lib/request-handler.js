...
module.exports = function processRequest (options, request, response) {
    ...
    const pipeDefinition = options.pipeDefinition;
    ...
    contextPromise.then((context) => {
        ...
        const resultStream = new StringifierStream((tag) => {
            ...
            if (tag.name === options.fragmentTag) {
                const fragment = new Fragment(tag, context, index++, requestFragment);
        ...
        resultStream.on('finish', () => {
            ...
            if (shouldWriteHead) {
                shouldWriteHead = false;
                this.emit('response', request, 200, responseHeaders);
