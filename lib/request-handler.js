...
module.exports = function processRequest (options, request, response) {
    ...
    contextPromise.then((context) => {
        ...
        if (typeof options.onBeforeEnd === 'function') {
            const assets = {
                css: [],
                js: [],
            };

            // capture contents to precache from all fragments
            this.on('fragment:response', (fragmentReq, fragment, statusCode, headers) => {
                ...
                if (links.stylesheet && links.stylesheet.url) {
                    assets.css.push(links.stylesheet.url);
                }
                if (links['fragment-script'] && links['fragment-script'].url) {
                    assets.js.push(links['fragment-script'].url);
                }
            });

            resultStream.on('alldone', () => {
                options.onBeforeEnd(assets);
            });
        }
