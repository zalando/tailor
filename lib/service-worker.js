...
function saveContent(store, data) {
    const update = {
        css: actual => store.set(
            CSS_KEY,
            [...new Set([...actual, ...data.css])].sort()
        ),
        js: actual => store.set(
            JS_KEY,
            [...new Set([...actual, ...data.js])].sort()
        )
    };
...
function handler(options, request, response) {
    ...
    if (isObject(options.serviceWorker.varNames)) {
    ...
    return Promise.all([
        ...
        response.end(`
            var ${cssVarName} = [${css.map(c => JSON.stringify(c))}];
            var ${jsVarName} = [${js.map(c => JSON.stringify(c))}];
